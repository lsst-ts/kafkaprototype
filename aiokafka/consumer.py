...
class AIOKafkaConsumer:
    ...
    def __del__(self, _warnings=warnings):
        if self._closed is False:
            _warnings.warn(
                f"Unclosed AIOKafkaConsumer {self!r}", ResourceWarning, source=self
            )
            context = {"consumer": self, "message": "Unclosed AIOKafkaConsumer"}
    ...
    async def start(self):
        ...
        * Join group if ``group_id`` provided
        ...
        if self._isolation_level == "read_committed" and self._client.api_version < (
            0,
            11,
        ):
